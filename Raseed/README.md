...
الخطوط والأيقونات مضمّنة داخل البرنامج (لا تحتاج تثبيتاً): **IBM Plex Sans Arabic** (رخصة OFL) وأيقونات **Lucide** كمسارات متجهة (رخصة ISC) — ملفات الرخص في `Assets/`.
...
| سقف الذمة (دينار/دولار، تنبيه أو منع) وفترة التسديد | الحسابات ← حساب الزبائن ← زر «سقف الذمة» |
| سند قبض وسند دفع بالدينار والدولار مع الخصم والمبلغ كتابةً | السندات ← سند قبض / سند دفع (اكتب رقم السند ثم Enter لفتح سند سابق) |
| الشعار وصورة ترويسة المطبوعات، ونسب الربح، وإيقاف الأنظمة غير المستخدمة | الأدوات ← الإعدادات ← إعدادات النظام |
| الكفلاء | الحسابات ← حساب الكفلاء |
| ثلاثة أسعار (جملة/مفرد/خاص) | تعريف المادة + مستوى سعر الزبون |
| سقف الذمة | زر «سقف الذمة» في حساب الزبائن («منع التعامل» يتطلب صلاحية التجاوز) |
...
## ما الجديد في الإصدار 8 — الوضوح والتصميم على كل الشاشات

![القائمة الجانبية المصغّرة](docs/sidebar-rail.png)

- **دقة العرض:** البرنامج أصبح «واعيًا للدقة» (System DPI Aware) بدل تكبير ويندوز المشوّش؛ كل الأبعاد والرسومات تُكبَّر مع إعداد التكبير (100% حتى 200%) فتبقى الخطوط حادة. جُرّب على 1366×768 بتكبير 100%، و1920×1080 بتكبير 125% و150%.
- **الأيقونات:** تُرسم الآن كأشكال متجهة مضمّنة في الكود (من Lucide) بدل خط الأيقونات، فتظهر على كل أجهزة ويندوز مهما كانت إعدادات الخطوط.
- **الخط:** IBM Plex Sans Arabic يُستخرج إلى `%LOCALAPPDATA%\Raseed\fonts` ويُسجَّل للبرنامج فقط (GDI وGDI+)، مع طريقة احتياطية من الذاكرة، ويُكتب سبب أي فشل في `fonts.log`.
- **القائمة الجانبية:** تصميم جديد فاتح ومنظم: مجموعات (العمليات اليومية، الحسابات والتقارير، الإدارة)، أيقونة ملونة لكل قسم، خط إرشاد للشاشات داخل القسم، وتمييز هادئ للشاشة الحالية. زر القائمة أعلى الشاشة يصغّرها إلى **شريط أيقونات**، والنقر على القسم يفتح **قائمة منبثقة** بشاشاته (ويُحفظ الاختيار).
- **شاشات متجاوبة:** صفوف النماذج تتمدد مع عرض البطاقة، وينزل العنوان فوق الحقل في المساحات الضيقة، وتلتف الأزرار بدل أن تُقص؛ تبويبات المواد في سطر واحد بعناوين مختصرة عند الضيق؛ بطاقات الوصول السريع والمؤشرات تتوزع حسب العرض.
- **القوائم المنسدلة** تُرسم بشكل موحد (النص والسهم) فلا يظهر سهم النظام ولا يُقص النص.
- **قائمة البيع:** المجاميع وأزرار الحفظ ظاهرة دائمًا؛ الحقول الإضافية (العنوان، مركز الكلفة، التوصيل، الملاحظات) تُطوى تلقائيًا في الشاشات القصيرة ويمكن إظهارها بزر «المزيد»، ولوحة معلومات المادة تختفي في الشاشات الضيقة.

## ما الجديد في الإصدار 7

![سند قبض](docs/receipt.png)

- **سند قبض وسند دفع** بالترتيب المألوف: رقم السند وتاريخه، اسم الحساب والصندوق، المبلغ والخصم **بالدينار والدولار** مع **المبلغ كتابةً** (تفقيط)، ولوحة ملونة بالرصيد السابق والحالي (مدين/لنا أو دائن/علينا). يظهر تأريخ آخر تسديد وآخر حركة، مع «كشف الحساب» و«تقرير السندات»، وخيارا «طباعة» و«نسخة العميل». السند برقم واحد يُعدَّل ويُحذف كاملًا.
- **سقف الذمة / فترة التسديد** لكل حساب: السقف بالدينار والدولار (أو موحّد)، «التنبيه عند التعامل» أو «منع التعامل» عند التجاوز، وتنبيه «تأخر التسديد» إذا لم يسدد الحساب خلال المدة — في الرئيسية وعلى الجرس.
- **الإعدادات:** الشعار وصورة ترويسة القوائم (تظهر في كل المطبوعات والتقارير)، المدينة والنشاط التجاري، **نسب الربح** (مفرد/جملة/خاص) لاقتراح أسعار البيع من سعر الشراء، و**أنظمة البرنامج**: إيقاف ما لا يحتاجه المحل (الأقساط، الشركات، الرقم التسلسلي، تعدد الصناديق...) يُخفي شاشاته لتبقى القائمة بسيطة.
- **قسم الأدوات:** الطابعة الافتراضية، الإعدادات، إعدادات التقارير، النسخ الاحتياطي، مدير المهام، التعريفات، سجل التحديثات، الاختصارات، الدعم، الحاسبة.
